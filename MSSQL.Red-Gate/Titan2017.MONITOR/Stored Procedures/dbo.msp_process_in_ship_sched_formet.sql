...
CREATE PROCEDURE [dbo].[msp_process_in_ship_sched_formet]
AS
---------------------------------------------------------------------------------------
...
--	--			27 NOV 2018, ASB, FT, LLC  - Created to isolate Formet ship schedule Processing
--
--	Returns:	  0	success
...
--	5. Get the totcount from the m_in_ship_schedule_formet table
...
--	5. Insert m_in_ship_schedule_formet and check the totcount from the m_in_ship_schedule_formet table

IF OBJECT_ID('tempdb..#FormetShipSchedules', 'U') IS NOT NULL
/*Then it exists*/
DROP TABLE #FormetShipSchedules

CREATE TABLE #FormetShipSchedules
	(
	...
	INSERT #FormetShipSchedules
		...
		SELECT
          ...
          release_dt =  convert(DATETIME,(CASE WHEN convert(tinyint,left(fr.ShipDate,2))<97  THEN '20' + fr.ShipDate
                         			 ELSE '19' + fr.ShipDate 
									END ))
		
		FROM
		dbo.edi_Formet862_Releases fr
		JOIN
		...
		UPDATE Frel
		SET Frel.Quantity = ( SELECT SUM(quantity) FROM #FormetShipSchedules frel2 WHERE frel2.shipto_id = Frel.shipto_id AND frel2.customer_part = frel.customer_part  AND frel2.Rowid<= frel.Rowid) 
		FROM #FormetShipSchedules Frel
...
INSERT dbo.m_in_ship_schedule_formet
		...
		FROM
		#FormetShipSchedules fr

		--SELECT * FROM #FormetShipSchedules

SELECT	@totcount = count ( 1 )
  from	m_in_ship_schedule_formet
...
--	53. Remove processed inbound data.
EXEC	msp_adjust_planning_862_formet
DELETE	m_in_ship_schedule_formet
DELETE	dbo.edi_Formet862_AccumATH
DELETE	dbo.edi_Formet862_AccumSHP
DELETE	dbo.edi_Formet862_Header
DELETE	dbo.edi_Formet862_Releases


SELECT DISTINCT 'Formet Ship Schedule', message FROM dbo.log
WHERE spid = @@spid 
