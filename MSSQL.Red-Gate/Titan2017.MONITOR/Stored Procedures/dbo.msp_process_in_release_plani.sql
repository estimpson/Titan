...
--	17. If this is a new order, delete old release plan and forecast 
--          schedule.
...
--	50. Close the cursor.
--	51. No inbound release plan to process, indicate in log and return rows 
--          not found.
...
+ ',destination :' + isnull(@shipto,'null destination') + ', customer po :' + isnull(@customerpo,'null customer po') 
+ ' & model year :'+ isnull(@modelyear,'null model year')+'.  Processing release #  (' + isnull(@releaseno,'null release number')+') due ' 
+ convert ( varchar(20), isnull(@releasedt,'null release date'), 113) + '.'
...
--	32. Calculation was unsuccessful, indicate in log
						...
						select	@@spid,
							(	select	isnull ( max ( id ), 0 ) + 1
								  from	log
								 where	spid = @@spid ),
							'Failed to calculated standard quantity for part:  ' + isnull(@blanketpart,'null blanket part')
							+ ' and unit:  ' + isnull(@orderunit,'null unit') + '.  Invalid unit for part.'
...
--	36. Create release.
				insert	order_detail_inserted (order_no,
						sequence,
						part_number,
						type,
						quantity,
...
--	37. Release was created, indicate in log.
				...
				select	@@spid,
					(	select	isnull ( max ( id ), 0 ) + 1
						  from	log
						 where	spid = @@spid ),
					'Inserted release for customer part :' + isnull(@customerpart,'null customer part')
					+', destination :' + isnull(@shipto,'null destination') + ', release date :' + convert( varchar(16), isnull(@releasedt,'null release date') ) 
					+ ', quantity :' + convert( varchar(20), isnull(@releasequantity,'null release quantity') )
...
--	41. Multiple orders found, indicate in log.
				...
				select	@@spid,
					...
					+ ', destination: ' + isnull(@shipto,'null destination') + ', customer po: ' + isnull(@customerpo,'null customer po') 
					+ ' & model year: ' + isnull(@modelyear,'null model year') + '. create one & then re-process.'
...
--+ ', destination: ' + @shipto + ', customer po: ' + @customerpo + ' & model 
--year: ' + @modelyear + '. create one & then re-process.'
