...
CREATE PROCEDURE [dbo].[msp_process_in_release_plan_modatek]
AS
-------------------------------------------------------------------------------------
...
--			16 MAR 2019, ASB, FT, LLC  - Created to isolate Modatek Release Processing
--
--	Returns:	  0	success
...
--	5. Get the totcount from the m_in_release_plan_modatek table
...
--	5. Insert m_in_release_plan_modatek and check the totcount from the m_in_release_plan_modatek table

IF OBJECT_ID('tempdb..#modatekReleases', 'U') IS NOT NULL
/*Then it exists*/
DROP TABLE #modatekReleases

CREATE TABLE #modatekReleases
	(
	...
	INSERT #modatekReleases
		...
		UPDATE Frel
		SET Frel.Quantity = ( SELECT SUM(quantity) FROM #modatekReleases frel2 WHERE frel2.shipto_id = Frel.shipto_id AND frel2.customer_part = frel.customer_part  AND frel2.Rowid<= frel.Rowid) 
		FROM #modatekReleases Frel
...
SELECT	@totcount = count ( 1 )
  from	m_in_release_plan_modatek
...
--	53. Remove processed inbound data.
EXECUTE msp_adjust_planning_830_modatek
DELETE	m_in_release_plan_modatek
DELETE	dbo.edi_modatek830_AccumATH
DELETE	dbo.edi_modatek830_AccumSHP
DELETE	dbo.edi_modatek830_Header
DELETE	dbo.edi_modatek830_Releases


SELECT DISTINCT 'Modatek Planning Release', message FROM dbo.log
WHERE spid = @@spid AND
log.message LIKE '%Blanket order does%' OR log.message LIKE '%Duplicate%'
UNION ALL
SELECT DISTINCT 'Modatek Planning Release', ' 830 Modatek Release Processing Completed'
