...
CREATE procedure [dbo].[EDI_GMDESADV_Detail_New] (@Shipper INT)
...
--dbo.EDI_GMDESADV_Detail_new 78965
...
SELECT	 at.serial,
		...
		COALESCE('KLT42', nullif(ppm.name,''), '0000PALT'), /*use pm.name Titan 3/13/13*/
		COALESCE('KLT42', nullif(pm.name,''), '0000CART'), /*use pm.name Titan 3/13/13*/
		(case when COALESCE(pm0.returnable,'N') = 'Y'  then '4' else '4' end ),                        
		at.part,
		at.quantity,
...
Select	CONVERT( char(80), '') AS PartialComplete,
		...
		CONVERT( char(80), COALESCE(substring(shipper.destination,2,10) , NULLIF(edi_setups.parent_destination,''), shipper.destination,'')) AS Destination, /* Substring for Titan 3/13/13 */
	...
	INSERT	#DESADVFlatFileLines (LineData)
	Select	('02'+ LEFT(DateShipped,12) /*+ '132'+ LEFT(ArrivalDate,12) + '203'*/  ) FROM #DESADVHeaderRaw	 /*Removed Arrival Date 3/13/13 */
	INSERT	#DESADVFlatFileLines (LineData)
	...
	INSERT	#DESADVFlatFileLines (LineData)
	select	('06' + LEFT(TransQual,2) + LEFT(TransMode,3) + LEFT(SCAC,17) + LEFT(Code182,3) /*+ LEFT(AETCNumberReason,3) + LEFT(AETCNumberResponsibility,3) + LEFT(AETCNumber,17)*/) from #DESADVHeaderRaw /*Titan 3/13/13*/
	INSERT	#DESADVFlatFileLines (LineData)
