...
--	13. Processing release, indicate in log.
		...
		select	@@spid,
			(	select	isnull ( max ( id ), 0 ) + 1
				  from	log
				 where	spid = @@spid ),
			'Searching for blanket order for customer part :  (' + @customerpart + ', 
destination :' + @shipto + ', customer po :' + @customerpo + ' & model year 
:'+ @modelyear+'.  Processing release #  (' + @releaseno+') due ' + convert 
( varchar(20), @releasedt, 113) + '.'
...
char ( 8 ), @prevorderno ) + '.  Order not found.'
				end -- (6B)
...
' and unit:  ' + @orderunit + '.  Invalid unit for part.'
...
--	37. Release was created, indicate in log.
				...
				select	@@spid,
					(	select	isnull ( max ( id ), 0 ) + 1
						  from	log
						 where	spid = @@spid ),
					'Inserted release for customer part :' + @customerpart+', destination 
:' + @shipto + ', release date :' + convert( varchar(16), @releasedt ) + ', 
quantity :' + convert( varchar(20), @releasequantity )
...
--	41. Multiple orders found, indicate in log.
				...
				select	@@spid,
					(	select	isnull ( max ( id ), 0 ) + 1
						  from	log
						 where	spid = @@spid ),
					'Blanket order is not unique for the customer part: ' + @customerpart + 
...
--	42. No orders found, indicate in log.
				...
				select	@@spid,
					(	select	isnull ( max ( id ), 0 ) + 1
						  from	log
						 where	spid = @@spid ),
					'Blanket order does not exist for the customer part: ' + @customerpart 
+ ', destination: ' + @shipto + ', customer po: ' + @customerpo + ' & model 
year: ' + @modelyear + '. create one & then re-process.'
