...
CREATE PROCEDURE [dbo].[msp_process_in_release_plan_formet]
AS
-------------------------------------------------------------------------------------
...
--			27 NOV 2018, ASB, FT, LLC  - Created to isolate Formet Release Processing
--
--	Returns:	  0	success
...
--	5. Get the totcount from the m_in_release_plan_formet table
...
--	5. Insert m_in_release_plan_formet and check the totcount from the m_in_release_plan_formet table

IF OBJECT_ID('tempdb..#FormetReleases', 'U') IS NOT NULL
/*Then it exists*/
DROP TABLE #FormetReleases

CREATE TABLE #FormetReleases
	(
	...
	INSERT #FormetReleases
		...
		UPDATE Frel
		SET Frel.Quantity = ( SELECT SUM(quantity) FROM #FormetReleases frel2 WHERE frel2.shipto_id = Frel.shipto_id AND frel2.customer_part = frel.customer_part  AND frel2.Rowid<= frel.Rowid) 
		FROM #FormetReleases Frel
...
--	53. Remove processed inbound data.
EXECUTE msp_adjust_planning_830_formet
DELETE	m_in_release_plan_formet
DELETE	dbo.edi_Formet830_AccumATH
DELETE	dbo.edi_Formet830_AccumSHP
DELETE	dbo.edi_Formet830_Header
DELETE	dbo.edi_Formet830_Releases


SELECT DISTINCT 'Formet Planning Release', message FROM dbo.log
WHERE spid = @@spid AND
