...
create function [EDI_XML_PAF_GM].[GetDESADV]
(	@ShipperID int
,	@Purpose char(2)
,	@PartialComplete int
)
			...
			(	select
					(	select
						...
						,	(	select
								...
								,	EDI_XML_EDIFACT.SEG_RFF(@dictionaryVersion, 'CN', ah.SealNumber)
						...
						,	(	select
								...
								,	EDI_XML_EDIFACT.SEG_CPS(@dictionaryVersion, al.RowNumber, null, 4)
								,	(	select
								 			EDI_XML.LOOP_INFO('PAC')
										,	EDI_XML_EDIFACT.SEG_PAC4(@dictionaryVersion, ap.PackCount)
										,	(	select
										 			EDI_XML.LOOP_INFO('PCI')
												,	EDI_XML_EDIFACT.SEG_PCI(@dictionaryVersion, '16')
												,	EDI_XML_EDIFACT.SEG_RFF(@dictionaryVersion, 'CN', ah.PRONumber)
										 		for xml raw ('LOOP-PCI'), type
										 	)
								 		from
								 			EDI_XML_PAF_GM.ASNPackages ap
										where
