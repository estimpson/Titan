CREATE TABLE [EDIFACT97A].[ReleasePlanSupplemental]
(
[Status] [int] NOT NULL CONSTRAINT [DF__ReleasePl__Statu__0BD1B136] DEFAULT ((0)),
[Type] [int] NOT NULL CONSTRAINT [DF__ReleasePla__Type__0CC5D56F] DEFAULT ((0)),
...
[RowCreateDT] [datetime] NULL CONSTRAINT [DF__ReleasePl__RowCr__0DB9F9A8] DEFAULT (getdate()),
[RowCreateUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__ReleasePl__RowCr__0EAE1DE1] DEFAULT (suser_name()),
[RowModifiedDT] [datetime] NULL CONSTRAINT [DF__ReleasePl__RowMo__0FA2421A] DEFAULT (getdate()),
[RowModifiedUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__ReleasePl__RowMo__10966653] DEFAULT (suser_name())
) ON [PRIMARY]
GO
...
create trigger [EDIFACT97A].[tr_ReleasePlanSupplemental_uRowModified] on [EDIFACT97A].[ReleasePlanSupplemental] after update
as
declare
	@TranDT datetime
,	@Result int
...
begin try
	...
	if	not update(RowModifiedDT) begin
		--- <Update rows="*">
		set	@TableName = 'EDIFACT97A.ReleasePlanSupplemental'
		...
		from
			EDIFACT97A.ReleasePlanSupplemental rps
			join inserted i
...
*/
GO
ALTER TABLE [EDIFACT97A].[ReleasePlanSupplemental] ADD CONSTRAINT [PK__ReleaseP__FFEE7451E6EE7861] PRIMARY KEY CLUSTERED  ([RowID]) ON [PRIMARY]
GO
