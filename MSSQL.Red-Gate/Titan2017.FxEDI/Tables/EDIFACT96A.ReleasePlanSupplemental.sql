CREATE TABLE [EDIFACT96A].[ReleasePlanSupplemental]
(
[Status] [int] NOT NULL CONSTRAINT [DF__ReleasePl__Statu__13A7DD28] DEFAULT ((0)),
[Type] [int] NOT NULL CONSTRAINT [DF__ReleasePla__Type__149C0161] DEFAULT ((0)),
...
[RowCreateDT] [datetime] NULL CONSTRAINT [DF__ReleasePl__RowCr__1590259A] DEFAULT (getdate()),
[RowCreateUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__ReleasePl__RowCr__168449D3] DEFAULT (suser_name()),
[RowModifiedDT] [datetime] NULL CONSTRAINT [DF__ReleasePl__RowMo__17786E0C] DEFAULT (getdate()),
[RowModifiedUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__ReleasePl__RowMo__186C9245] DEFAULT (suser_name())
) ON [PRIMARY]
GO
...
create trigger [EDIFACT96A].[tr_ReleasePlanSupplemental_uRowModified] on [EDIFACT96A].[ReleasePlanSupplemental] after update
as
declare
	@TranDT datetime
,	@Result int
...
begin try
	...
	if	not update(RowModifiedDT) begin
		--- <Update rows="*">
		set	@TableName = 'EDIFACT96A.ReleasePlanSupplemental'
		...
		from
			EDIFACT96A.ReleasePlanSupplemental rps
			join inserted i
...
*/
GO
ALTER TABLE [EDIFACT96A].[ReleasePlanSupplemental] ADD CONSTRAINT [PK__ReleaseP__FFEE7451F6785AD0] PRIMARY KEY CLUSTERED  ([RowID]) ON [PRIMARY]
GO
