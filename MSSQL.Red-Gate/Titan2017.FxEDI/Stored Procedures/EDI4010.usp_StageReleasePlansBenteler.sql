...
create procedure [EDI4010].[usp_StageReleasePlansBenteler]
...
begin
	...
	begin try
		...
		begin
			...
			update
				rp
			set	rp.ReleaseNo = rtrim(rp.CustomerPOLine) + '!' + rtrim(rp.CustomerPO) + '^' + rtrim(rp.CustomerPORelease) + '$' + coalesce(nullif(rp.CustomerECL, '--'), '')
			,	rp.CustomerECL = nullif(rp.CustomerECL, '--')
			from
				@ReleasePlans rp
...
execute
	@ProcReturn = EDI4010.usp_StageReleasePlansBenteler
	@TranDT = @TranDT out
