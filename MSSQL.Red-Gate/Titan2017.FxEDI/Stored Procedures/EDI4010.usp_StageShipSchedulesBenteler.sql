...
create procedure [EDI4010].[usp_StageShipSchedulesBenteler]
...
begin
	...
	begin try
		...
		begin
			...
			,	CustomerPORelease = LOOP_LIN.Data.value('(for $a in SEG-LIN/DE[@code="0235"] where $a="RN" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			,	CustomerECL = LOOP_LIN.Data.value('(for $a in SEG-LIN/DE[@code="0235"] where $a="EC" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			,	CustomerModelYear = LOOP_LIN.Data.value('(for $a in SEG-LIN/DE[@code="0235"] where $a="MY" return $a/../DE[. >> $a][@code="0234"][1])[1]', 'varchar(30)')
			...
			update
				ss
			set	ss.ReleaseNo = rtrim(ss.CustomerPOLine) + '!' + rtrim(ss.CustomerPO) + '^' + rtrim(ss.CustomerPORelease) + '$' + coalesce(nullif(ss.CustomerECL, '--'), '')
			,	ss.CustomerECL = nullif(ss.CustomerECL, '--')
			from
				@ShipSchedules ss

			if	@Debug & 0x01 = 0x01 begin
				select '@ShipSchedules', * from @ShipSchedules ss
			end
		...
		begin
			...
			,	ss.CustomerECL
			,	ValueQualifier = ss.Data.value('(/LOOP-RFF/DE[@code="7511"])[1]', 'varchar(50)')
			,	Value = ss.Data.value('(/LOOP-RFF/DE[@code="7102"])[1]', 'varchar(50)')
			from
				@ShipSchedules ss
		...
		begin
			...
			,	ReceivedQty = LOOP_SHP.Data.value('(for $a in SEG-SHP/DE[@code="0673"] where $a="02" return $a/../DE[. >> $a][@code="0380"][1])[1]', 'varchar(30)')
...
execute
	@ProcReturn = EDI4010.usp_StageShipSchedulesBenteler
	@TranDT = @TranDT out
