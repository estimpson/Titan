...
create procedure [EDIFACT96A].[usp_ProcessInReleases]
	@Test int = 1
...
begin
	...
	begin try
		...
		begin
			...
			from
				EDIFACT96A.CurrentShipSchedules() css
			...
			from
				EDIFACT96A.CurrentReleasePlans() crp
		...
		begin
			...
			from
				EDIFACT96A.ShipScheduleReleases ssr
			...
			if	@Debug & 0x01 = 0x01 begin
				select * from EDIFACT96A.ShipScheduleHeaders ssh
				select * from EDIFACT96A.ReleasePlanHeaders rph
			end
		...
		begin
			...
			from
				@CurrentShipSchedules css
				join EDIFACT96A.BlanketOrders bo
					on bo.BlanketOrderNo = css.SalesOrderNumber
				join EDIFACT96A.ShipScheduleHeaders ssh
					on ssh.RawDocumentGUID = css.RawDocumentGUID
				left join EDIFACT96A.ShipScheduleAccums ssa
				...
				left join EDIFACT96A.ShipScheduleAuthAccums ssaa
			...
			from
				...
				join EDIFACT96A.ReleasePlanHeaders rph
					on rph.RawDocumentGUID = crp.RawDocumentGUID
				left join EDIFACT96A.ReleasePlanAccums rpa
				...
				left join EDIFACT96A.ReleasePlanAuthAccums rpaa
			...
			from
				...
				join EDIFACT96A.ReleasePlanHeaders rph
					on rph.RawDocumentGUID = crp.RawDocumentGUID
				join EDIFACT96A.ReleasePlanReleases rpr
		...
		begin
			...
			from
				...
				join EDIFACT96A.ReleasePlanSupplemental rps
			...
			from
				...
				join EDIFACT96A.ShipScheduleSupplemental sss
		...
		begin
			...
			from
				...
				join EDIFACT96A.ShipScheduleAccums ssa
					on ssa.RawDocumentGUID = css.RawDocumentGUID
					   and ssa.CustomerPart = css.CustomerPart
					   and ssa.ShipToCode = css.ShipToCode
					   and coalesce(ssa.CustomerPO, '') = coalesce(css.CustomerPO, '')
					   and coalesce(ssa.CustomerModelYear, '') = coalesce(css.CustomerModelYear, '')
				join EDIFACT96A.ShipScheduleAuthAccums ssaa
					on ssaa.RawDocumentGUID = css.RawDocumentGUID
					   and ssaa.CustomerPart = css.CustomerPart
					   and ssaa.ShipToCode = css.ShipToCode
					   and coalesce(ssaa.CustomerPO, '') = coalesce(css.CustomerPO, '')
			...
			from
				...
				join EDIFACT96A.ReleasePlanAccums rpa
					on rpa.RawDocumentGUID = crp.RawDocumentGUID
					   and rpa.CustomerPart = crp.CustomerPart
					   and rpa.ShipToCode = crp.ShipToCode
					   and coalesce(rpa.CustomerPO, '') = coalesce(crp.CustomerPO, '')
					   and coalesce(rpa.CustomerModelYear, '') = coalesce(crp.CustomerModelYear, '')
				join EDIFACT96A.ReleasePlanAuthAccums rpaa
					on rpaa.RawDocumentGUID = crp.RawDocumentGUID
					   and rpaa.CustomerPart = crp.CustomerPart
					   and rpaa.ShipToCode = crp.ShipToCode
					   and coalesce(rpaa.CustomerPO, '') = coalesce(crp.CustomerPO, '')
		...
		begin
			...
			declare
				@emailHeader nvarchar(max) = N'EDI Processing for EDIFACT96A'
			declare
...
execute
	@ProcReturn = EDIFACT96A.usp_ProcessInReleases
	@Test = 1
,	@TranDT = @TranDT out
