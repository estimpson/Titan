...
create procedure [EDI4010].[usp_ProcessInReleases]
	@Test int = 1
...
begin
	...
	begin try
		...
		begin
			...
			from
				EDI4010.CurrentShipSchedules() css
			...
			from
				EDI4010.CurrentReleasePlans() crp
		...
		begin
			...
			from
				EDI4010.ShipScheduleReleases ssr
			...
			from
				EDI4010.ReleasePlanReleases rpr
			...
			if	@Debug & 0x01 = 0x01 begin
				select * from EDI4010.ShipScheduleHeaders ssh
				select * from EDI4010.ReleasePlanHeaders rph
			end
		...
		begin
			...
			from
				...
				left join EDI4010.ShipScheduleAccums ssa
				...
				left join EDI4010.ShipScheduleAuthAccums ssaa
			...
			from
				...
				join EDI4010.ShipScheduleHeaders ssh
					on ssh.RawDocumentGUID = css.RawDocumentGUID
				join EDI4010.ShipScheduleReleases ssr
			...
			from
				...
				join EDI4010.ReleasePlanHeaders rph
					on rph.RawDocumentGUID = crp.RawDocumentGUID
				left join EDI4010.ReleasePlanAccums rpa
				...
				left join EDI4010.ReleasePlanAuthAccums rpaa
			...
			from
				...
				join EDI4010.ReleasePlanHeaders rph
					on rph.RawDocumentGUID = crp.RawDocumentGUID
				join EDI4010.ReleasePlanReleases rpr
		...
		begin
			...
			if	@Debug & 0x01 = 0x01 begin
				select * from Fx.order_detail od where od.order_no in (select OrderNo from @RawReleases rr) order by od.order_no, od.sequence
			end
			...
			if	@Debug & 0x01 = 0x01 begin
				...
				,	NewWeeklyDemand = sum(rr.QtyRelease) / (datediff(week, min(rr.ReleaseDT), max(rr.ReleaseDT)) + 1)
				,	NewWeeks = datediff(week, min(rr.ReleaseDT), max(rr.ReleaseDT)) + 1
				,	OldOrderQty =
						(	select
								sum(odd.quantity)
							from
				...
				,	OldWeeklyDemand =
						(	select
								sum(odd.quantity) / (datediff(week, min(odd.due_date), max(odd.due_date)) + 1)
							from
								Fx.order_detail_deleted odd
							where
								odd.order_no = rr.OrderNo
				...
				,	OldWeeks =
						(	select
								datediff(week, min(odd.due_date), max(odd.due_date)) + 1
							from
								Fx.order_detail_deleted odd
							where
								odd.order_no = rr.OrderNo
			...
			if	@Debug & 0x01 = 0x01 begin
				select * from FxTest.order_detail od where od.order_no in (select OrderNo from @RawReleases rr) order by od.order_no, od.sequence
			end
		...
		begin
			...
			from
				...
				join EDI4010.ReleasePlanSupplemental rps
		...
		begin
			...
			from
				...
				join EDI4010.ShipScheduleAccums ssa
					on ssa.RawDocumentGUID = css.RawDocumentGUID
					   and ssa.CustomerPart = css.CustomerPart
					   and ssa.ShipToCode = css.ShipToCode
					   and coalesce(ssa.CustomerPO, '') = coalesce(css.CustomerPO, '')
					   and coalesce(ssa.CustomerModelYear, '') = coalesce(css.CustomerModelYear, '')
				join EDI4010.ShipScheduleAuthAccums ssaa
					on ssaa.RawDocumentGUID = css.RawDocumentGUID
					   and ssaa.CustomerPart = css.CustomerPart
					   and ssaa.ShipToCode = css.ShipToCode
					   and coalesce(ssaa.CustomerPO, '') = coalesce(css.CustomerPO, '')
			...
			from
				...
				join EDI4010.ReleasePlanAccums rpa
					on rpa.RawDocumentGUID = crp.RawDocumentGUID
					   and rpa.CustomerPart = crp.CustomerPart
					   and rpa.ShipToCode = crp.ShipToCode
					   and coalesce(rpa.CustomerPO, '') = coalesce(crp.CustomerPO, '')
					   and coalesce(rpa.CustomerModelYear, '') = coalesce(crp.CustomerModelYear, '')
				join EDI4010.ReleasePlanAuthAccums rpaa
					on rpaa.RawDocumentGUID = crp.RawDocumentGUID
					   and rpaa.CustomerPart = crp.CustomerPart
					   and rpaa.ShipToCode = crp.ShipToCode
					   and coalesce(rpaa.CustomerPO, '') = coalesce(crp.CustomerPO, '')
		...
		begin
			...
			declare
				@emailHeader nvarchar(max) = N'EDI Processing for EDI4010'
			declare
...
execute
	@ProcReturn = EDI4010.usp_ProcessInReleases
	@Test = 1
,	@TranDT = @TranDT out
